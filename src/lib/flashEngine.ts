...
import type { FlashBackup, FlashSector } from '@/types';
...
export interface DMESector {
  name: string;
  startAddress: number;     // Physical flash address
  size: number;             // Bytes
  isBootSector: boolean;    // Contains bootloader - never flash this
  isCalibration: boolean;   // Contains tune calibration data
  isProgram: boolean;       // Contains program code
  checksumOffset?: number;  // Offset within sector where checksum lives
...
export const DME_SECTORS_MSD80: DMESector[] = [
  ...
  // Calibration sectors - these contain the tune data
  ...
  // Full calibration block for quick flash
...
export const DME_SECTORS_MSD81: DMESector[] = [
  // MSD81 has same layout as MSD80 but different memory addresses
  { name: 'Bootloader', startAddress: 0x00000000, size: 0x10000, isBootSector: true, isCalibration: false, isProgram: false, checksumType: 'none', description: 'DME boot code - NEVER WRITE', writable: false },
  { name: 'Program_0', startAddress: 0x00010000, size: 0x20000, isBootSector: false, isCalibration: false, isProgram: true, checksumType: 'bmw_crc16', description: 'Program code block 0', writable: true },
  { name: 'Program_1', startAddress: 0x00030000, size: 0x20000, isBootSector: false, isCalibration: false, isProgram: true, checksumType: 'bmw_crc16', description: 'Program code block 1', writable: true },
  { name: 'Program_2', startAddress: 0x00050000, size: 0x20000, isBootSector: false, isCalibration: false, isProgram: true, checksumType: 'bmw_crc16', description: 'Program code block 2', writable: true },
...
export interface ParsedFlashFile {
  ...
  fileSize: number;
  data: Uint8Array;
  ecuType: string;        // Detected ECU type from file
...
/**
 * Parse a flash file (.bin) and validate its contents.
 * Detects ECU type, verifies checksums, and identifies all sectors.
 */
  ...
  return new Promise((resolve, reject) => {
    ...
    reader.onload = () => {
      try {
        ...
        const result = analyzeFlashData(file.name, data);
        resolve(result);
      } catch (e) {
        reject(e);
      }
...
function analyzeFlashData(fileName: string, data: Uint8Array): ParsedFlashFile {
  ...
  // Expected sizes: 2MB (full), 512KB (calibration), 1MB (IJE0/MSD8x subset)
  ...
  // Detect ECU type from internal identifiers
  ...
  for (const sector of sectorLayout) {
    ...
    if (sector.checksumOffset && sector.startAddress + sector.checksumOffset < size) {
      ...
      // Try to find checksum at end of sector data
  ...
  // Check for boot sector overwrite protection
  if (size >= 0x10000) {
    const bootData = data.slice(0, 0x10000);
    const hasBootSignature = checkBootSignature(bootData);
    if (hasBootSignature) {
  ...
  return {
    ...
    vin,
    sectors,
    checksumsValid: allChecksumsValid,
    warnings,
  };
}
...
/**
 * BMW CRC-16 (used in MSD80/MSD81)
 * Polynomial: 0x8005, Initial: 0xFFFF, Reflect: true for both input and output
 */
...
/**
 * CRC-16 CCITT (used in some BMW modules)
...
/**
 * 32-bit sum checksum (simple addition)
 */
...
// ============================================================================
// FLASH FILE UTILITIES
// ============================================================================

/**
 * Extract calibration data only (512KB from offset 0x90000 in a 2MB file)
 */
export function extractCalibration(data: Uint8Array): Uint8Array {
  if (data.length === 0x80000) {
    // Already calibration only
    return data;
  }
  if (data.length >= 0x110000) {
    return data.slice(0x90000, 0x110000);
  }
  return data;
}

/**
 * Recompute and update all checksums in a flash file
 */
export function fixChecksums(data: Uint8Array, ecuType: string): Uint8Array {
  ...
  for (const sector of sectors) {
    ...
    // Write checksum at specified offset or end of sector
    const checksumAddr = sector.checksumOffset
      ? sector.startAddress + sector.checksumOffset
      : endAddr - 4;
...
/**
 * Compare two flash files and show differences
 */
export function diffFlashFiles(original: Uint8Array, modified: Uint8Array): { address: number; original: number; modified: number }[] {
  ...
  for (let i = 0; i < minLen; i++) {
    if (original[i] !== modified[i]) {
      diffs.push({ address: i, original: original[i], modified: modified[i] });
    }
  }
...
/**
 * Build flash operations for a given flash type
 */
export function buildFlashOperations(
  flashFile: ParsedFlashFile,
  flashType: 'full' | 'quick' | 'live',
  ecuType: string
): FlashOperation[] {
  const sectors = ecuType.includes('MSD81') ? DME_SECTORS_MSD81 : DME_SECTORS_MSD80;
  ...
  if (flashType === 'quick' || flashType === 'live') {
    // Quick flash: only calibration sectors
    ...
    for (const sector of calSectors) {
      if (sector.startAddress < flashFile.data.length) {
        const end = Math.min(sector.startAddress + sector.size, flashFile.data.length);
        operations.push({
          sector,
          data: flashFile.data.slice(sector.startAddress, end),
          verifyAfterWrite: true,
        });
      }
    }
  } else {
    // Full flash: all writable sectors except bootloader
    for (const sector of sectors) {
...
export function buildFlashBackup(sectorData: { name: string; data: Uint8Array; checksum: string }[], vin: string, ecuType: string, softwareVersion: string): FlashBackup {
  ...
  const sectors: FlashSector[] = sectorData.map(s => ({
    name: s.name,
...
function detectECUType(data: Uint8Array): string {
  // Search for ECU type strings in the flash
...
function extractVIN(data: Uint8Array, fileSize: number): string {
  // VIN is typically stored at offset 0x94000 in the calibration area
  ...
  for (const offset of vinOffsets) {
    ...
    // Validate: VIN should be 17 chars with only alphanumeric
...
function extractSoftwareVersion(data: Uint8Array, fileSize: number): string {
  ...
  for (const offset of searchOffsets) {
    ...
    // Software version typically looks like "SW_XXX" or version numbers
...
function checkBootSignature(data: Uint8Array): boolean {
  // Bootloader typically has specific signature bytes at start
  // Check for ARM Cortex-M3 vector table pattern
  if (data.length < 8) return false;
  const stackPointer = readUint32(data, 0);
  const resetVector = readUint32(data, 4);
  // Valid stack pointer should be in RAM range (0x40000000-0x4FFFFFFF for some BMW DMEs)
...
export const FLASH_CONSTANTS = {
  DME_FLASH_SIZE: 0x200000,        // 2MB total flash
  CALIBRATION_SIZE: 0x80000,       // 512KB calibration region
  CALIBRATION_START: 0x90000,      // Calibration starts here
  BOOTLOADER_SIZE: 0x10000,        // 64KB bootloader
  SECTOR_SIZE: 0x20000,            // 128KB per sector
  MAX_FLASH_TIME_MS: 120000,       // 2 minute max flash time
  QUICK_FLASH_TIME_MS: 5000,       // 5 seconds for quick flash
  LIVE_FLASH_TIME_MS: 3000,        // 3 seconds for live flash
  MIN_BATTERY_VOLTAGE: 13.0,       // Min 13.0V for flashing
  MIN_BATTERY_VOLTAGE_LIVE: 13.5,  // Min 13.5V for live flash
};
